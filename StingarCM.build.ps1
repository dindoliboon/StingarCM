...
task TestCode {
    ...
    $TestResult = Invoke-Pester -Script "$PSScriptRoot\Test" -Tag Unit -Show 'Header','Summary' -PassThru

    if($TestResult.FailedCount -gt 0) {
        Write-Warning -Message "Failing Tests:"
        $TestResult.TestResult.Where{$_.Result -eq 'Failed'} | ForEach-Object -Process {
            Write-Warning -Message "$($_.Describe) $($_.Context) $($_.Name) $($_.StackTrace)"
...
task TestBuild {
    ...
    $Script =  @{Path="$PSScriptRoot\Test"; Parameters=@{ModuleBase=$Script:CompileResult.ModuleBase}}
    ...
    $CodeCoverageResult | Sort-Object SourceFile, Line | Format-Table
}
